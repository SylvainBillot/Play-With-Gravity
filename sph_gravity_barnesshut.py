...
def main():
    """
    Main simulation function to run cosmological N-body simulation.
    """
    global R, L, N, G, soft, h, tho0, k, mu, D, VNF, H0, IE, Omega_m, Omega_L
    global a0, td_factor, epsilon, mass_min, mass_max, dotsize, rho_min, rho_max, cmap, norm, rng
    global pos, mass, vel
    global pe, time_list, t_sim, a_factor
    ...
    D = 1e-5 # densité de la matière
    VNF = 1.0 # factor to increase the force when particles are very near
                ...
                for oy in range(-1, 2):
                    ...
                    for oz in range(-1, 2):
                        ...
                        for idx in range(cstart[cid], cstart[cid + 1]):
                            ...
                            # Fusion 
                            fe = calculer_force_evitement_fast(pos[i], pos[j], vel[i], vel[j], d_min, VNF)
                            fx += fe[0]
                            fy += fe[1]
                            fz += fe[2]
